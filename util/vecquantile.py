...
class QuantileVector:
    ...
    Add any amount of segmentation_data repeatedly via add(segmentation_data).  At any time,
    ...
    Good for computing quantiles of huge segmentation_data without using much memory.
    Works well on arbitrary segmentation_data with probability near 1.
    ...
    def __init__(self, depth=1, resolution=24 * 1024, buffersize=None, dtype=None, seed=None):
    ...
    def _expand(self):
        ...
        for index in range(1, len(self.data)):
            # Scan for existing segmentation_data that needs to be moved down a level.
            amount = self.firstfree[index]
            if amount == 0:
                continue
            position = self.firstfree[index - 1]
            # Move segmentation_data down if it would leave enough empty space there
            ...
            if self.data[index - 1].shape[1] - (amount + position) >= (
                ...
                # Scrunch the segmentation_data if it would not.
    ...
    def quantiles(self, quantiles, old_style=False):
        ...
        cum_weights = numpy.cumsum(weights, axis=-1) - weights / 2
        if old_style:
            # To be convenient with numpy.percentile
            cum_weights -= cum_weights[:, 0:1]
            cum_weights /= cum_weights[:, -1:]
        else:
            cum_weights /= numpy.sum(weights, axis=-1, keepdims=True)
        result = numpy.empty(shape=(self.depth, len(quantiles)))
        for d in range(self.depth):
            result[d] = numpy.interp(quantiles, cum_weights[d], summary[d])
        return result
...
if __name__ == "__main__":
    ...
    # segmentation_data[::2] = segmentation_data[-2::-2]
    # numpy.random.shuffle(segmentation_data)
    starttime = time.time()
